function [smsorts,smFDs]=smomcurve(sorts,number,smorange)
%smooth data
for smi=1:97
    ma0=max(find(sorts(:,smi)==0));
    ma1=max(find(sorts(:,smi)==1));
    if ma1>=ma0
        smsorts(1:ma0,smi)=smooth(sorts(1:ma0,smi),smorange);
        smsorts(ma0:ma1,smi)=smooth(sorts(ma0:ma1,smi),smorange);
        smsorts(ma1:length(sorts(:,smi)),smi)=smooth(sorts(ma1:length(sorts(:,smi)),smi),smorange);
    else
        ...
        if isempty(locs)
            ...
            if isempty(locs2)
                smsorts(:,smi)=smooth(sorts(:,smi),smorange);
            elseif ma0>ma1             
                smsorts(1:ma1,smi)=smooth(sorts(1:ma1,smi),smorange);
                smsorts(ma1:ma0,smi)=smooth(sorts(ma1:ma0,smi),smorange);
                smsorts(ma0:length(sorts(:,smi)),smi)=smooth(sorts(ma0:length(sorts(:,smi)),smi),smorange);
            else
                smsorts(:,smi)=smooth(sorts(:,smi),smorange);
            end
        end
